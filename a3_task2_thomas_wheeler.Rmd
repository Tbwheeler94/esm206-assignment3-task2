...
##Note regarding line 49: The last part of the name was dropped, because there were only 2 columns to capture the split name values and this person had 2 dashes in their name.
```
...
```{r}
# Your code here:
sw_sep_skin <- starwars %>% # Start from starwars data
  separate(col = skin_color, # Separate the `skin` column
...
```{r}
# Your code here: 
my_dates <- c(11061972, 02142007, 01301843) # Creates a vector of dates w/o spaces
my_dates_converted <- lubridate::mdy(my_dates) # Converts to Date, stores as my_dates_converted
my_dates_converted # Return my_dates_converted
...
gasol_hookshots <- lakers %>% #Assign dataframe name 'gasol_hookshots'
  filter(player=="Pau Gasol", type=="hook") %>% #filter by Pau Gasol's hook shots
  mutate(date = ymd(date)) %>% #Convert integer date values to date
  mutate(month = month(date)) %>% #Create column with just month values
  group_by(month) %>% # Group Gasol's hook shots by month
  count(month) %>%  #Count Gasol's hook shots by month
  rename(count_by_month = n) #Rename column of monthly totals
...
```{r}
# Your code here: 
storm_danger <- storms %>% # Start from storms data
  mutate(danger = case_when( # Add a new column danger, which contains... 
    status == 'hurricane' ~ "high", # "high" if hurricane
    TRUE ~ "moderate" # "moderate" if not a hurricane
  )
  )
```
